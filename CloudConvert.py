...
class ConversionProcess():
    ...
    def init(self, fromfile, tofile, fromformat=None, toformat=None, options=None):
        """
    ...
    def start(self, wait=False):
        ...
        If `wait` is set to True, this method will block until the
        conversion is completed.
        """
        _options = self.options.copy()

        # TODO: Test if this can timeout the request
        if wait:
            _options["wait"] = "True"

        CloudConvert.upload(
            self.fromfile, self.toformat, self.url, _options
        )
